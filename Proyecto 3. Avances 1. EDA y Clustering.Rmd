...
Para este punto ya se descargo año por año el .sav (formato SPSS) de la página [Instituto Nacional de Estadística - Guatemala](https://www.ine.gob.gt/vitales/), de nacimiento de bebes del país (Guatemala), para su posterior análisis.
...
# Crear lista vacía para resumen
...
for (archivo in archivos) {
  ...
  tryCatch({
    ...
    message("❌ Error leyendo ", basename(archivo))
  })
}
...
for (archivo in archivos) {
  ...
  tryCatch({
    ...
    # Verifica que el data frame tenga nombres de columna válidos
    if (!is.null(names(df))) {
      resumen <- tibble(
        archivo = basename(archivo),
        variable = names(df),
      ...
      message("⚠️ Sin nombres de columnas: ", basename(archivo))
    }

    rm(df)
    gc()
  }, error = function(e) {
    message("❌ Error con ", basename(archivo), ": ", e$message)
  })
}

# Unir resultados válidos
...
**1.a. Descripción del conjunto de datos**
...
- **Años analizados:** 14 (2009 a 2022)
...
A continuación se muestra una clasificación de los tipos de variables más comunes en los archivos:

|**Tipo de Variable**           |**Ejemplos de variables**                   |**Descripción**                                     |
|-------------------------------|--------------------------------------------|----------------------------------------------------|
| **Categóricas nominales**     | `Sexo`, `Tipar`, `Asisrec`, `Sitioocu`     | Identifican categorías sin orden específico        |
| **Categóricas ordinales**     | `Escolap`, `Escolam`, `Escivp`, `Escivm`   | Categorías ordenadas (ej. niveles de educación)    |
| **Numéricas discretas**       | `Edadp`, `Edadm`, `Libras`, `Onzas`        | Variables contables como edad y peso al nacer      |
| **Ubicación geográfica**      | `Depreg`, `Mupreg`, `Depocu`, `Mupocu`     | Departamento y municipio de registro y ocurrencia  |
| **Información del padre**     | `Ocupap`, `Escolap`, `Depnap`, `Naciop`    | Datos laborales y educativos del padre             |
| **Información de la madre**   | `Ocupam`, `Escolam`, `Edadp`, `Naciom`     | Datos laborales, educativos y de edad de la madre  |
| **Identificadores temporales**| `Mesreg`, `Añoreg`, `Mesocu`, `Añoocu`     | Fechas de registro y ocurrencia del nacimiento     |
...
for (archivo in archivos) {
  ...
  tryCatch({
    ...
    # Conversión segura solo de columnas disponibles
    df <- df %>%
      mutate(
    ...
    # FILTRAR VALORES IMPOSIBLES / ATÍPICOS
    ...
    if (length(vars_num_disponibles) > 0) {
      ...
      for (var in vars_num_disponibles) {
        print(ggplot(df, aes_string(x = var)) +
                ...
                labs(title = paste("Distribución de", var, "en", basename(archivo))))
      }
    } else {
      cat("\n⚠️ No se encontraron variables numéricas disponibles.\n")
    }
    ...
    if (length(vars_cat_disponibles) > 0) {
      ...
      for (cat_var in vars_cat_disponibles) {
        cat("\n🔸 Frecuencia de", cat_var, ":\n")
        print(table(df[[cat_var]], useNA = "ifany"))
      }
    } else {
      cat("\n⚠️ No se encontraron variables categóricas disponibles.\n")
    }

    # Observaciones
    cat("\n📝 Observaciones sugeridas:\n")
    cat("- Las variables numéricas han sido limpiadas de outliers y ceros inválidos.\n")
    cat("- Evalúa si las distribuciones son normales.\n")
    cat("- Observa los valores más frecuentes por categoría.\n")
    cat("\n🔍 Preguntas:\n")
    cat("- ¿Las variables numéricas tienen forma de campana (normal)?\n")
    cat("- ¿Existen valores atípicos?\n")
    cat("- ¿Qué categorías son más comunes en sexo, tipo de parto, asistencia médica?\n")
    cat("- ¿Hay diferencias llamativas en los años?\n")
...
for (archivo in archivos) {
  ...
  tryCatch({
    ...
    if (!all(c("Sexo", "Libras", "Onzas", "Añoreg") %in% names(df))) {
    ...
    df <- df %>%
      ...
      filter(peso_total > 1, peso_total <= 10)  # Filtrar pesos válidos

    # Agrupar por año y sexo para calcular el peso promedio
    df_resumen <- df %>%
      group_by(Añoreg, Sexo) %>%
      summarise(peso_promedio = mean(peso_total, na.rm = TRUE), .groups = "drop")

    # Gráfica de barras con el peso promedio
    p <- ggplot(df_resumen, aes(x = Sexo, y = peso_promedio, fill = Sexo)) +
      stat_summary(fun = mean, geom = "bar", position = "dodge") +
      labs(title = "Peso promedio al nacer según el sexo del bebé",
           x = "Sexo del bebé",
           y = "Peso al nacer (libras)") +
      theme_minimal()

    # Asegúrate de imprimir la gráfica
    print(p)
  ...
  tryCatch({
    ...
    if ("Edadp" %in% names(df) && "peso_total" %in% names(df)) {
      print(ggplot(df, aes(x = Edadp, y = peso_total)) +
              ...
              labs(title = paste("Relación entre Edad de la madre y Peso al nacer en", basename(archivo)),
...
Ahora nos haremos la siguiente pregunta y es la de si la escolaridad de la madre influye en el peso que tienen los hijos de ella. 
...
for (archivo in archivos) {
  ...
  tryCatch({
    ...
    # 📊 Influencia de la escolaridad en el peso del bebé
    if ("Escolap" %in% names(df) && "peso_total" %in% names(df)) {
      print(ggplot(df, aes(x = Escolap, y = peso_total, fill = Escolap)) +
              geom_boxplot() +
              labs(title = paste("Influencia de la escolaridad de la madre en el peso del bebé en", basename(archivo)),
                   x = "Nivel educativo de la madre", y = "Peso al nacer (libras)") +
              theme_minimal())
    }

    # 📊 Influencia de la escolaridad en la edad de la madre
    if ("Escolap" %in% names(df) && "Edadp" %in% names(df)) {
      print(ggplot(df, aes(x = Escolap, y = Edadp, fill = Escolap)) +
              geom_boxplot() +
              labs(title = paste("Influencia de la escolaridad de la madre en la edad al tener el bebé en", basename(archivo)),
                   x = "Nivel educativo de la madre", y = "Edad de la madre (años)") +
              theme_minimal())
    }
...
Podemos ver que si hay una tendencia a que la escolaridad de la madre afecta al peso de los bebes , pero sucede algo curioso que el nivel alto de escolaridad se sale de la tendencia que el nivel de escolaridad haga que el peso de los niños sea menor.

Tambien si observamos que el nivel educativo de la madre la tendencia es de bajo y tambien de una escolaridad de 9. Lo que indica que una gran parte de las madres o tienen muy baja escolaridad o muy alta, y tambien se puede observar que la mayoria de las que son jovenes son las que tienen escolaridades bajas y altas y las que tienen escolaridad de entre 5 y 6 son personas mas grandes. 

Esto puede ser un indicador del nivel socieconomico de la madre y como este influye  a que dad suele tener a un bebe. 
